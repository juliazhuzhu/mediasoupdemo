...
export default class RoomClient
{
			...
			switch (request.method)
			{
				case 'newConsumer':
				{
					...
					try
					{
						...
						logger.warn('newConsumer consumer id %s, appData.share :%o', id,appData);
						if (this._e2eKey && e2e.isSupported())
						{
						...
						if (consumer.kind === 'video' && appData.share == undefined){
							logger.warn('newConsumer record video consumer id %s', consumer.id);
							this._videoConsumerId = consumer.id;
							consumer.type = 'webcam';
						}else {
							consumer.type = 'share';
							//type = 'content';
						}
						...
						logger.warn('newConsumer consumer consumer %o, temporalLayers :%o', consumer,temporalLayers);
						
						if (appData.share && consumer.kind === 'video'){
							//logger.warn('newConsumer remove video consumer %s', this._videoConsumerId);
							//await store.dispatch(stateActions.removeConsumer(this._videoConsumerId, peerId));
						}
	...
	async enableWebcam()
	{
		...
		try
		{
			if (!this._externalVideo)
			{
				...
				logger.warn('enableWebcam() resolution %s', resolution);
				if (!device)
			...
			const codecOptions =
			{
				videoGoogleStartBitrate : 5000000,
				videoGoogleMaxBitrate : 10000000,
	...
	async changeWebcamResolution()
	{
		...
		try
		{
			...
			logger.debug('changeWebcamResolution() | _webcam %o', this._webcam);
			const stream = await navigator.mediaDevices.getUserMedia(
				{
					video :
					{
	...
	async enableShare()
	{
		...
		try
		{
			...
			track = stream.getVideoTracks()[0];
			logger.debug('enableShare() track %o', track);
			let encodings;
			let codec;
			const codecOptions =
			{
	...
	async _joinRoom()
	{
		...
		try
		{
			...
			logger.warn('routerRtpCapabilities %o',routerRtpCapabilities)		
			...
			if (this._consume)
			{
				...
				logger.warn('transportInfo :%o',transportInfo);
